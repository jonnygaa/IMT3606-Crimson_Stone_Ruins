...
@onready var spawn: DungeonRoom3D = $Spawn
...
@onready var exit: DungeonRoom3D = $Exit
...
@export var default_seed : int = 3282746692
...
func generate(seed : int = int(generate_seed) if generate_seed.is_valid_int() else randi()) -> void:
	...
	var path = base_path + "level_" + str(GameManager.level_id) + "/"
	...
	if spaw_exit_pairs.size() > 0:
		# Pick a random placement pair
		var id = randi_range(0, spaw_exit_pairs.size() - 1)
		var spawn_pos: Vector3 = spaw_exit_pairs[id].spawn_pos
		var spawn_y_rot: int = spaw_exit_pairs[id].spawn_y_rot if spaw_exit_pairs[id].spawn_y_rot >= 0 else 360 + spaw_exit_pairs[id].spawn_y_rot
		var exit_pos: Vector3 = spaw_exit_pairs[id].exit_pos
		var exit_y_rot: int = spaw_exit_pairs[id].exit_y_rot if spaw_exit_pairs[id].exit_y_rot >= 0 else 360 + spaw_exit_pairs[id].exit_y_rot
		
		spawn.virtual_transform.origin = spawn_pos
		spawn.room_rotations = spawn_y_rot / 90 # Divide by 90 to get a number between 0 (0 degree) and 3 (270 degrees)
		exit.virtual_transform.origin = exit_pos
		exit.room_rotations = exit_y_rot / 90
	
		# Sets a new default seed for this specific positing of the two rooms.
		default_seed = spaw_exit_pairs[id].default_seed
...
# Connects trigger to generate a new dungeon
func connect_new_dungeon_signal():
	for trigger in get_tree().get_nodes_in_group("exit_zone"):
		trigger.generate_new_dungeon.connect(_generate_new_dungeon)
