...
func save() -> Dictionary:
	...
	# Gets all the chlidren of the main CSGBox so all actives doors plus the CSGBox making the room space
	# Handles older way of making the rooms (for tha labyrinth)
	if has_node("CSGBox3D"):
		for child in $CSGBox3D.get_children():
			names_to_keep.append(child.name)
			...
			if child.get_child_count() > 0:
				for wall in child.get_children():
					if wall.get_child_count() > 0:
						...
						names_to_keep.append(name + "/" + wall.name + "/" + door.name)
