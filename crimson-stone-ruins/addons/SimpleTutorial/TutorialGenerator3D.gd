@tool
class_name TutorialGenerator3D
extends Node3D
...
# Function signature should be Callable(room_instances : Array[TutorialRoom3D], rng : RandomNumberGenerator) -> Array[TutorialRoom3D]
...
## Each one must inherit TutorialRoom.
## Not necessary to place the corridor room here. Place that in the corridor_room_scene property.
@export var room_scenes : Array[PackedScene] = []
## The corridor room is a special room scene which must be a 1x1x1 (in voxels) scene inheriting TutorialRoom which is used to connect all the placed rooms.
@export var corridor_room_scene : PackedScene

## Tutorial grid size measured in voxel units, voxel size is chosen in the voxel_scale property.
...
## If the voxel scale is different, the rooms will be scaled (perhaps non uniformly) to match the TutorialGenerator's voxel scale.
...
## Generate the dungeon in editor. If you added any custom rooms which extend TutorialRoom,
...
@onready var spawn: TutorialRoom3D = $Spawn

# Variable to place the spawn and the exit at the start of the generation
# Don't need to re-declare spawn here as it is just above
@onready var exit: TutorialRoom3D = $Exit

var dungeon_manager: TutorialManager = null

@export_group("Default seed")
# Seed that will be used in case that all the generation attemps failed
@export var default_seed : int = 1000000000
...
func add_debug_view_if_not_exist():
	if not _debug_view:
		_debug_view = preload("res://addons/SimpleTutorial/debug_visuals/DungeonGenerator3DDebugView.gd").new()
...
func _process(delta):
	if Engine.is_editor_hint():
		...
		for c in get_children():
			if c is TutorialRoom3D and not c.virtualized_from:
...
var room_instances : Array[TutorialRoom3D]
var corridor_room_instance : TutorialRoom3D
var iterations := 0
...
func generate(seed : int = default_seed) -> void:
	print("generating :)")
	if is_currently_generating:
		_printerr("SimpleTutorial Error: Tutorial currently generating, cannot generate.")
		return
	
	stage = BuildStage.NOT_STARTED
	
	if not validate_dungeon():
		_printerr("SimpleTutorial Error: Cannot generate.")
		return
	
	# Spawn and exit placement
	var base_path = "res://resources/spawn_exit_pairs/"
	var dir = DirAccess.open(base_path)
	if dir == null:
		printerr("Failed to open directory: ", base_path)
		return

	# Create the level subfolder path
	var path = base_path + "tutorial/"
	
	var spaw_exit_pairs: Array[SpawnExitPair] = []
	# Retrieve each placements
	for file in DirAccess.get_files_at(path):
		if file.get_extension() == "tres":
			var pair: SpawnExitPair = load(path+file)
			spaw_exit_pairs.append(pair)
	# Pick a spesific placement pair
	var pair: SpawnExitPair = spaw_exit_pairs.front()

	spawn.virtual_transform.origin = pair.spawn_pos
	spawn.room_rotations = pair.spawn_y_rot / 90

	exit.virtual_transform.origin = pair.exit_pos
	exit.room_rotations = pair.exit_y_rot / 90

	
	# Sets a new default seed for this specific positing of the two rooms.
	default_seed = spaw_exit_pairs[0].default_seed

	rng = RandomNumberGenerator.new()
	rng.seed = seed
	print("TutorialGenerator3D generate(): Using seed ", seed)
	...
	if not setup_room_instances_and_validate_before_generate():
		_fail_generation("TutorialGenerator3D generation failed while setting up rooms.")
		return
...
func _run_generate_loop(first_call : bool = true) -> void:
	...
	while retry_attempts <= max_retries + 1 and not full_abort_triggered:
		...
		if retry_attempts > max_retries + 1:
			...
			if retry_attempts > max_retries:
				# If the max generation attempts is reached, we change the seed to one that we know works and generate one last time.
				rng.seed = default_seed
				_printwarning("Now using default seed: "+str(default_seed))
...
func _fail_generation(error : String = "Aborted generation") -> void:
	_printerr("SimpleTutorial Error: ", error)
	_printerr("SimpleTutorial Error: Failed to generate dungeon")
	failed_to_generate = true

func abort_generation():
	if not is_currently_generating:
		_printwarning("TutorialGenerator3D not currently generating.")
		return
	failed_to_generate = true
...
func _dungeon_finished_generating() -> void:
	...
	if player_scene == null:
		push_warning("player_scene not set!")
	else:
		# if it's the first generation, we need to instantiate the player, then we keep a reference of him
		if player_instance == null:
			player_instance = player_scene.instantiate()
			add_child(player_instance)
		player_instance.set_new_position(spawn.get_spawn_position())
	...
	for sprite in get_tree().get_nodes_in_group("face_player_sprite"):
		sprite.global_rotation.y = player_instance.global_rotation.y
...
# Emit done signals for dungeon & place_room for all TutorialRooms.
func _emit_done_signals():
	...
	for room in _rooms_placed:
		if not room.original_ready_func_called:
			_printwarning("_ready not called on "+room.name+". Room placement, finalization, and doors will be broken. Make sure to call super._ready() at the top of your _ready func when inheriting TutorialRoom3D.")
		room.dungeon_done_generating.emit()
	for preplaced_room in find_children("*", "TutorialRoom3D", false):
		preplaced_room.dungeon_done_generating.emit()
	print("TutorialGenerator3D finished generating.")
	for room in room_instances:
		room.queue_free()
...
var _rooms_placed : Array[TutorialRoom3D]
var _custom_rand_rooms : Array[TutorialRoom3D]
	...
	if first_call_in_loop:
		...
		if custom_get_rooms_function is Callable:
			...
			if not _custom_rand_rooms is Array or len(_custom_rand_rooms) == 0:
				_fail_generation("custom_get_rooms_function takes should return a non-empty Array of TutorialRoom3Ds.")
				_printwarning("custom_get_rooms_function takes (room_instances : Array[TutorialRoom3D], rng_seeded : RandomNumberGenerator) as the arguments and should use .create_clone_and_make_virtual_unless_visualizing() to clone and then position with .set_position_by_grid_pos(Vector3i) or .rotation = 0 through 3 for number of 90 degree y rotations for the room.")
				return
			for room in _custom_rand_rooms:
				if not room is TutorialRoom3D:
					_fail_generation("custom_get_rooms_function supplied an object that is not a TutorialRoom3D. Ensure all rooms supplied inherit TutorialRoom3D, and use the @tool annotation if generating in editor.")
					return
				if room_instances.find(room) != -1:
					_fail_generation("custom_get_rooms_function supplied a room instance without cloning it. Always use TutorialRoom3D.create_clone_and_make_virtual_unless_visualizing() to create room instances.")
					return
	
	var rand_room : TutorialRoom3D
...
# Array [TutorialRoom3D, grid_pos_y] for where to place rooms
...
class StairRoomInfo:
	...
	func _init(room_instance : TutorialRoom3D):
	...
	# format: [[TutorialRoom3D, grid_pos_y], ...]. returns [] if no valid way to connect those floors
...
func _find_stair_chain_to_connect_floors(floor_1 : int, floor_2 : int, floor_graph : TutorialTreeGraph, stair_info_dict : Dictionary) -> Array:
	var stair_chain := []
	var bottom_floor : int = min(floor_1, floor_2)
	var top_floor : int = max(floor_1, floor_2)
...
# returns Array with elements in format [room : TutorialRoom3D, room_grid_pos_y : int]
func _make_and_solve_floors_graph() -> Array:
	...
	var floors_tree_graph := TutorialTreeGraph.new()
	var add_room_to_floors_graph := (func(room : TutorialRoom3D, room_y_pos : int):
	...
	var stairs_to_add : Array = [] # element format: [TutorialRoom3D, grid_pos_y]
	while not floors_tree_graph.is_fully_connected():
		...
		for f1 in floors_tree_graph.get_all_nodes():
			for f2 in floors_tree_graph.get_all_nodes().filter(func(_f2): return _f2 > f1):
				...
				for s in stair_info_dict.values():
					# Just looking for 1 valid element [TutorialRoom3D, grid_pos_y]
		...
		_fail_generation("Failed to connect all floors together with stairs. Ensure you have at least 1 TutorialRoom3D with 'is_stair_room' set to true with 2 or more doors leading different floors. Simplest is a 2 floor room with 1 door on each floor.")
...
func connect_rooms_iteration(first_call_in_loop : bool) -> void:
	if first_call_in_loop:
		...
		_astar3d = TutorialAStar3D.new(self, _quick_room_check_dict, _quick_corridors_check_dict)
...
func get_rooms_less_than_min_count(include_stairs : bool):
	return room_instances.filter(func(room : TutorialRoom3D):
		if not include_stairs and room.is_stair_room:
			return false
		var already_in_tree = _rooms_placed.filter(func(placed_room : TutorialRoom3D):
...
func get_randomly_positioned_room() -> TutorialRoom3D:
	var room : TutorialRoom3D = get_rooms_less_than_max_count(false)[rng.randi() % get_rooms_less_than_max_count(false).size()]
	...
	room.room_rotations = 1 #rng.randi_range(0,3)
	room.set_position_by_grid_pos(rand_pos)
	return room

func place_room(room : TutorialRoom3D) -> void:
...
func cleanup_and_reset_dungeon_generator() -> void:
	if is_currently_generating:
		_fail_generation("Tutorial reset while generating.")
	if running_thread:
...
func setup_room_instances_and_validate_before_generate() -> bool:
	if not room_scenes:
		_printerr("SimpleTutorial Error: No TutorialRoom3D room scenes set.")
		return false
	var inst_arr : Array[TutorialRoom3D] = []
	for s in room_scenes:
		if not s: continue
		var inst = s.instantiate()
		if not inst is TutorialRoom3D:
			_printerr("SimpleTutorial Error: "+s.resource_path+" room scene does not inherit TutorialRoom3D. Also may need @tool annotation if generating in editor.")
			return false
		else:
			...
			inst_arr.append(inst as TutorialRoom3D)
...
func get_grid_aabbi() -> TutorialAABBi:
	return TutorialAABBi.new(Vector3i(0,0,0), dungeon_size)

func get_room_at_pos(grid_pos : Vector3i) -> TutorialRoom3D:
	if stage > BuildStage.CONNECT_ROOMS:
		...
		var room = quick_check if quick_check else _quick_corridors_check_dict.get(grid_pos)
		if room != null:
			return room
	for room in get_all_placed_and_preplaced_rooms():
...
func get_preplaced_rooms() -> Array:
	...
	if OS.get_thread_caller_id() != OS.get_main_thread_id():
		...
		rooms.assign(get_children().filter(func(c): return c is TutorialRoom3D))
...
func get_stair_rooms_from_instances() -> Array:
	...
	rooms.assign(room_instances.filter(func(c):
		if not c is TutorialRoom3D: return false
...
func validate_dungeon(error_callback = null, warning_callback = null) -> bool:
	...
	if not warning_callback is Callable: warning_callback = (func(str): _printwarning("SimpleTutorial Warning: ", str))
	if not error_callback is Callable: error_callback = (func(str): _printerr("SimpleTutorial Error: ", str))
	...
	if room_scenes.size() == 0:
		error_callback.call("No rooms added. Add TutorialRoom scenes to the room_scenes property.")
	...
	if room_instances is Array:
		for room in room_instances:
			if not room is TutorialRoom3D:
				error_callback.call("Room "+room.name+" does not inherit TutorialRoom3D! Also add @tool to dungeon room script if generating in editor.")
			else:
				if not room.has_method("validate_room"):
					error_callback.call("validate_room() method not found on room "+room.name+". Ensure it inherits TutorialRoom3D and has the @tool annotation if you're trying to generate in editor.")
				room.validate_room(error_callback, warning_callback)
	if not corridor_room_instance is TutorialRoom3D:
		var corridor_name = corridor_room_instance.name if corridor_room_instance else "null"
		error_callback.call("Corridor Room "+corridor_name+" does not inherit TutorialRoom3D!")
	if corridor_room_instance is TutorialRoom3D and corridor_room_instance.size_in_voxels != Vector3i(1,1,1):
	...
	if corridor_room_instance and not corridor_room_instance.has_method("get_doors"):
		error_callback.call("get_doors() method not found on the corridor room. Ensure it inherits TutorialRoom3D and has the @tool annotation if you're trying to generate in editor.")
	if corridor_room_instance is TutorialRoom3D and (corridor_room_instance.get_doors().size() != 4 or corridor_room_instance.get_doors().any(func(d): return not d.optional)):
		error_callback.call("Corridor Room must have 4 optional doors")
	if corridor_room_instance is TutorialRoom3D:
...
# Called by the signal to generate a new dungeon
func _generate_new_dungeon() -> void:
	dungeon_manager.next_dungeon()
