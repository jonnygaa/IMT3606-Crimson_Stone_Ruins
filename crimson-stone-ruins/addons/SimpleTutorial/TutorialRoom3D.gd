...
var dungeon_generator : TutorialGenerator3D :
	get:
		if is_inside_tree() and get_parent() is TutorialGenerator3D:
			return get_parent()
...
## Determines if the room is the starter room of a level.
@export var is_spawn_room := false
# Marker containing the transform spawn position
...
func _validate_property(property: Dictionary):
	if property.name in ["force_align_with_grid_button"] and not get_parent() is TutorialGenerator3D:
...
var was_preplaced = false :
	get:
		if Engine.is_editor_hint():
			return is_inside_tree() and get_parent() is TutorialGenerator3D
...
# So instead, we will create virtual TutorialRoom3Ds which reference the single original we create.
# Then, we can still encapsulate all the logic into this one class & improve performance.
var virtualized_from : TutorialRoom3D = null
...
var virtual_self : TutorialRoom3D = self :
...
func _ready():
	...
	if get_parent() is Node3D:
		if get_parent() is TutorialGenerator3D:
			dungeon_generator = get_parent()
		if get_parent().get_parent() is TutorialGenerator3D:
...
func copy_all_props(from : TutorialRoom3D, to : TutorialRoom3D) -> void:
...
func get_original_packed_scene() -> PackedScene:
	# Try to find the room packed scene in the TutorialGenerator3D so we don't have to load()
	...
	printerr(self.name+" Could not find TutorialRoom3D's original packed scene. This shouldn't happen. Are you manually spawning rooms?")
	return null

func create_clone_and_make_virtual_unless_visualizing() -> TutorialRoom3D:
...
# Spawn the real room into the TutorialGenerator
func unvirtualize_and_free_clone_if_needed(into_parent : Node3D) -> TutorialRoom3D:
	if not virtualized_from:
	...
	self.queue_free()
	var inst : TutorialRoom3D = get_original_packed_scene().instantiate()
...
func add_debug_view_if_not_exist():
	if not _debug_view:
		_debug_view = preload("res://addons/SimpleTutorial/debug_visuals/DungeonRoom3DDebugView.gd").new()
...
class Door:
	...
	var optional : bool
	var room : TutorialRoom3D
	var door_node : Node3D
	func _init(local_pos : Vector3, dir : DungeonUtils.Direction, optional : bool, room : TutorialRoom3D, door_node : Node3D):
	...
	func validate_door() -> bool:
		if not TutorialAABBi.new(Vector3i(), room.size_in_voxels).contains_point(local_pos):
			return false
		if TutorialAABBi.new(Vector3i(), room.size_in_voxels).contains_point(exit_pos_local):
			return false
		if find_duplicates().size() > 1:
			return false
		return true
	func get_room_leads_to() -> TutorialRoom3D:
...
func push_away_from_and_stay_within_bounds(other_room : TutorialRoom3D) -> void:
...
func overlaps_room(other_room : TutorialRoom3D) -> bool:
	...
	var door_intersects = (func(door : Door, room : TutorialRoom3D):
...
## Room must be scaled so voxel scale matches the TutorialGenerator3D's voxel scale.
...
func set_position_by_grid_pos(new_grid_pos : Vector3i = get_grid_pos()) -> void:
	if not dungeon_generator: printerr("set_position_by_grid_pos: No dungeon_generator set on TutorialRoom3D")
...
func xform_aabbi(aabbi : TutorialAABBi, xform : Transform3D) -> TutorialAABBi:
	return TutorialAABBi.from_AABB_rounded(xform_aabb(aabbi.to_AABB(), xform))

func get_local_aabb() -> AABB:
	var size := Vector3(size_in_voxels) * voxel_scale
	return AABB(-size/2.0, size)

func get_grid_aabbi(include_doors : bool) -> TutorialAABBi:
	...
	var grid_aabbi = TutorialAABBi.from_AABB_rounded(xform_aabb(get_local_aabb(), get_xform_to(SPACE.LOCAL_SPACE, SPACE.DUNGEON_GRID)))
...
func validate_room(error_callback = null, warning_callback = null) -> bool:
	...
	if voxel_scale != dungeon_generator.voxel_scale:
		warning_callback.call("Room "+self.name+"'s voxel scale does not match TutorialGenerator3D voxel size. Room will be scaled to match.")
	if size_in_voxels != size_in_voxels.clamp(Vector3i(0,0,0), dungeon_generator.dungeon_size):
		error_callback.call("Room "+self.name+" is too big for the TutorialGenerator3D!")
	
	return not any_errors["err"]
	
# Return the transform of the spawn point in the room
func get_spawn_position() -> Vector3:
	# Verifies that spawn_transform is set
	if spawn_transform == null:
		push_warning("spawn_transform parameter is not set!")
		return global_position # Pos of the middle of the room
		
	return spawn_transform.global_position
