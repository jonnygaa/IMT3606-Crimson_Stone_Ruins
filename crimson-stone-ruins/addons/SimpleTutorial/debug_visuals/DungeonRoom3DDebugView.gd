...
func update_door_visuals(show_visuals : bool):
	...
	while len(door_visuals) < len(room_doors):
		var door_visual = preload("res://addons/SimpleTutorial/debug_visuals/DoorDebugVisual.tscn").instantiate()
...
func update_visual():
	...
	#if not get_parent() is TutorialGenerator3D:
		#print(dungeon_room.name)
	if dungeon_room.dungeon_generator and dungeon_room.dungeon_generator.hide_debug_visuals_for_all_generated_rooms and not (dungeon_room.get_parent() is TutorialGenerator3D):
		show_editor = false
		show_game = false
	...
	if not wireframe_cube or not is_instance_valid(wireframe_cube):
		wireframe_cube = preload("res://addons/SimpleTutorial/debug_visuals/WireframeCube.tscn").instantiate()
	...
	if not aabb_with_doors:
		...
		aabb_with_doors.material = preload("res://addons/SimpleTutorial/debug_visuals/WireframeColorMat.tres")
		add_child(aabb_with_doors)
	var rel_room_aabb = dungeon_room.xform_aabb(dungeon_room.get_grid_aabbi(true).to_AABB(), dungeon_room.get_xform_to(TutorialRoom3D.SPACE.DUNGEON_GRID, TutorialRoom3D.SPACE.LOCAL_SPACE)).abs()
	...
	if not debug_alert or not is_instance_valid(debug_alert):
		debug_alert = preload("res://addons/SimpleTutorial/debug_visuals/DebugAlert.tscn").instantiate()
