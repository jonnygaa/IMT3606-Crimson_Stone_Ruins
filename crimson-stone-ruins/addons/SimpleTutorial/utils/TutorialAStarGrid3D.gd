...
var rooms_check_dict : Dictionary # Vector3i : TutorialRoom3D (Non-Corridor) instance
var corridors_check_dict : Dictionary # Vector3i : TutorialRoom3D (Corridor) instance

var cap_required_doors_phase := false

func can_walk_from_to(dungeon_generator : TutorialGenerator3D, pos_a : Vector3i, pos_b : Vector3i) -> bool:
...
func _init(dungeon_generator : TutorialGenerator3D, rooms_check_dict : Dictionary, corridors_check_dict : Dictionary):
...
func _estimate_cost(from_id : int, to_id : int) -> float:
	if dungeon_generator.astar_heuristic == TutorialGenerator3D.AStarHeuristics.NONE_DIJKSTRAS:
		return 0.0
	elif dungeon_generator.astar_heuristic == TutorialGenerator3D.AStarHeuristics.MANHATTAN:
	...
	elif dungeon_generator.astar_heuristic == TutorialGenerator3D.AStarHeuristics.EUCLIDEAN:
		var diff := get_point_position(to_id) - get_point_position(from_id)
...
func get_vec3i_path(from : Vector3i, to : Vector3i) -> Array[Vector3i]:
	if not vec3i_to_pt_id.has(from):
		printerr("❌ get_vec3i_path: Missing 'from' key: ", from)
		return []
	if not vec3i_to_pt_id.has(to):
		printerr("❌ get_vec3i_path: Missing 'to' key: ", to)
		return []
